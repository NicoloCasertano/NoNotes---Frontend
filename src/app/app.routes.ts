...
export const routes: Routes = [{ path: '', redirectTo: 'log-in-area', pathMatch: 'full'},
    ...
    { path: 'listening-area/:id', loadComponent: () => import('./components/listening-area/listening-area').then(m => m.ListeningArea)},
    { path: 'upload-work/:id', loadComponent: () => import('./components/upload-work/upload-work').then(m => m.UploadWork), canActivate: [RoleGuard], data: { roles: ['ROLE_ADMIN', 'ROLE_EMPLOYEE'] } },
    { path: 'work-list/:id', loadComponent: () => import('./components/home-lists/work-list/work-list').then(m => m.WorkList), canActivate: [RoleGuard]},
